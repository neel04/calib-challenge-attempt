...
#extract hevc for testing dataset (unlabeled)
def test_hevc_to_frames(sample_num, out_folder):
  if not Path(f'./{out_folder}').exists():
    execute_shell(f'mkdir {out_folder}')

  vidcap = cv2.VideoCapture(f'/content/calib-challenge-attempt/calib_challenge/unlabeled/{sample_num}.hevc')   # sample_num --> int
